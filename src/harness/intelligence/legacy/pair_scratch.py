"""Legacy PAIR scratch implementation (research/testing).

WARNING: This is a research/testing implementation that may differ from the official version.
For production use, install the official implementation:
    aipop plugins install pair

Official implementation achieves 88% jailbreak rate on Vicuna and 73% on Gemini.
...
warnings.warn(
    "Using legacy PAIR implementation. "
    "For production use with paper-validated results, install official implementation: "
    "aipop plugins install pair",
    DeprecationWarning,
    stacklevel=2,
)
...
class PAIRScratchConfig:
    ...
    keep_last_n: int = 4  # Truncate history to last N turns (official default)
    ...
    def attack(
        ...
        for stream in self.streams:
            ...
            for iteration in range(self.config.iterations_per_stream):
                ...
                # Get attacker's next prompt with retry logic (max 5 attempts like official PAIR)
                # Official code: AttackLM.get_attack() retries up to max_n_attack_attempts=5
                max_attempts = 5
                attacker_text = None
                improvement = ""
                new_prompt = None
                ...
                for attempt in range(max_attempts):
                    try:
                        # Invoke attacker adapter (attacker_prompt already contains full system + history)
                        attacker_response = self.attacker_adapter.invoke(attacker_prompt)
                        attacker_text = (
                        ...
                        # If we got a valid prompt, break
                        if new_prompt and new_prompt.strip():
                            break
                        else:
                            logger.warning(f"Stream {stream.stream_id} attempt {attempt + 1}/{max_attempts}: empty prompt")
                            
                    except Exception as e:
                        # Enhanced error handling with root cause analysis
                        from harness.utils.error_handling import extract_root_cause
                        
                        error_type, error_msg, suggestion = extract_root_cause(e)
                        
                        logger.warning(
                            f"Stream {stream.stream_id} attempt {attempt + 1}/{max_attempts} failed: "
                        ...
                        if attempt == max_attempts - 1:
                            ...
                            logger.error(
                                f"Stream {stream.stream_id} attacker failed after {max_attempts} attempts.\n"
                                f"  Error: {error_type}: {error_msg}\n"
                                f"  Suggestion: {suggestion}"
                            )
                            break
                
                # If all attempts failed, skip this stream and continue with others
                if not new_prompt or not new_prompt.strip():
                    logger.warning(f"Stream {stream.stream_id} produced no valid prompt, skipping stream")
                    break  # Break inner loop, continue to next stream
                ...
                try:
                    ...
                    # Log error but continue iteration (record as failed turn)
                    logger.error(f"Stream {stream.stream_id} target model failed: {e}")
                    target_text = f"[ERROR: {str(e)}]"
                    # Don't break - record this turn and continue
                ...
                # Truncate conversation history to avoid context overflow (official PAIR behavior)
                # Official code: conv.messages = conv.messages[-2*(keep_last_n):]
                # We keep last N turns
                if len(stream.history) > self.config.keep_last_n:
                    stream.history = stream.history[-self.config.keep_last_n:]
                    logger.debug(f"Stream {stream.stream_id} truncated history to last {self.config.keep_last_n} turns")
